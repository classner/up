...
from capsule_ch import Capsule, cap_f
...
joint2name = ['pelvis', 'leftThigh', 'rightThigh', 'spine', 'leftCalf',
              ...
              'head', 'leftUpperArm', 'rightUpperArm', 'leftForeArm',
              'rightForeArm', 'leftHand', 'rightHand']

# the orientation of each capsule
rots0 = ch.asarray([[0,       0,       np.pi/2],
                    [0,       0,       np.pi],
                    [0,       0,       np.pi],
                    [0,       0,       np.pi/2],
                    [0,       0,       np.pi],
...
collisions = [[0, 16], # hip and leftForeArm
             ...
             #[6, 14], # spine1 and leftUpperArm
             #[6, 15], # spine1 and rightUpperArm
             ...
             #[9, 14], # spine2 and leftUpperArm
             #[9, 15], # spine2 and rightUpperArm
...
def get_capsules(model, wrt_betas=None, length_regs=None, rad_regs=None):
    ...
    if wrt_betas is not None:
      # we must have the regressors
      assert(length_regs is not None and rad_regs is not None)
      # betas must be a chumpy object
      assert(hasattr(wrt_betas, 'dterms'))
      # WHY IS THIS 301 x 20 (and not 300)?
      pad = ch.concatenate((wrt_betas, ch.zeros(n_shape_dofs-len(wrt_betas)), ch.ones(1)))
      #lengths = ch.ch.MatVecMult(length_regs.T, pad)
      #rads = ch.ch.MatVecMult(rad_regs.T, pad)
    ...
    # the joint regressors are the original, pre-optimized ones (middle of the part frontier)
    myJ_regressor = model.J_regressor_prior# sp.csc_matrix((data, (rows, cols)))
    myJ0 = ch.vstack((ch.ch.MatVecMult(myJ_regressor, model.v_template[:,0] + model.shapedirs[:,:,:n_betas].dot(betas)[:,0]),
                      ch.ch.MatVecMult(myJ_regressor, model.v_template[:,1] + model.shapedirs[:,:,:n_betas].dot(betas)[:,1]),
                      ch.ch.MatVecMult(myJ_regressor, model.v_template[:,2] + model.shapedirs[:,:,:n_betas].dot(betas)[:,2]))).T
    # with small adjustments for hips, spine and feet
    myJ = ch.vstack([ch.concatenate([myJ0[0,0], (.6*myJ0[0,1]+.2*myJ0[1,1]+.2*myJ0[2,1]), myJ0[9,2]]),
                     ch.vstack([myJ0[i] for i in range(1,7)]),
                     ch.concatenate([myJ0[7,0], (1.1*myJ0[7,1] - .1*myJ0[4,1]), myJ0[7,2]]),
                     ch.concatenate([myJ0[8,0], (1.1*myJ0[8,1] - .1*myJ0[5,1]), myJ0[8,2]]),
                     ch.concatenate([myJ0[9,0], myJ0[9,1], (.2*myJ0[9,2]+.8*myJ0[12,2])]),
                     #ch.concatenate([myJ0[9,0], .9*myJ0[9,1] + .1*myJ0[12,1], myJ0[9,2]]),
                     ch.vstack([myJ0[i] for i in range(10,24)])])

    capsules = []
    # create one capsule per mujoco joint. Assign some parameters (irrelevant now with the regressors)
    for ijoint, segms in enumerate(mujoco2segm):
        if wrt_betas is None:
          ...
          rads[ijoint] = .5*((dims[(np.argmax(dims) + 1)%3] + dims[(np.argmax(dims) + 2)%3])/4.)
          lengths[ijoint] = max(dims) - 2.*rads[ijoint].r        
        # the core joints are different, since the capsule is not in the joint but in the middle        
        if ijoint in mujoco_t_mid:
            len_offset = ch.vstack([ch.zeros(1), ch.abs(lengths[ijoint])/2., ch.zeros(1)]).reshape(3,1)
            caps = Capsule((J_off[ijoint] + myJ[mujoco2segm[ijoint][0]]).reshape(3,1) -
                           #Rodrigues(rots[ijoint])[0].dot(len_offset),
                           Rodrigues(rots[ijoint]).dot(len_offset),
                           rots[ijoint], rads[ijoint], lengths[ijoint])
        else:
            caps = Capsule((J_off[ijoint] + myJ[mujoco2segm[ijoint][0]]).reshape(3,1), rots[ijoint], rads[ijoint], lengths[ijoint])
        caps.id = ijoint
...
def show_capsules(capsules, mv):
  ...
  fs = np.vstack([cap_f+(i*len(capsules[0].v)) for i in range(len(capsules_v))])
  mcap = Mesh(v=vs, f=fs)
  mv.set_static_meshes([mcap])


def set_sphere_centers(capsule, floor=True):
  if np.isnan(capsule.length.r)[0] or np.isnan(capsule.rad.r)[0]:
    n_spheres = 0
  else:
    if floor:
  ...
  if n_spheres >= 1:
    ...
    for i in xrange(n_spheres):
      centers.append(capsule.axis[0].r + (capsule.axis[1].r-capsule.axis[0].r)*step*(i+1)/capsule.length.r)
...
def capsule_dist(capsule0, capsule1, alpha=.3, increase_hand=True):
  range0 = range(capsule0.center_id, capsule0.center_id + len(capsule0.centers))
  range1 = range(capsule1.center_id, capsule1.center_id + len(capsule1.centers))

  #cnt0 = ch.hstack([[cid]*len(range1) for cid in range0])
  cnt0 = ch.concatenate([[cid]*len(range1) for cid in range0])
  #cnt1 = ch.hstack([range1]*len(range0))
  cnt1 = ch.concatenate([range1]*len(range0))
  # dst = capsule0.rad.r + capsule1.rad.r
  if increase_hand:
    if capsule0.id == 18 or capsule0.id == 19 or capsule1.id == 18 or capsule1.id == 19:
      dst = (alpha * 1.2 * capsule0.rad.r)**2 + (alpha * 1.2 * capsule1.rad.r)**2
    else:
...
def get_capsule_bweights(vs):
  ...
  caps_weights = np.asarray(sp.csc_matrix((data, (rows, cols)), shape=(vs.shape[0], 24)).todense())
  return caps_weights
...
def get_sphere_bweights(sph_vs, capsules):
  ...
  sph_weights = np.asarray(sp.csc_matrix((data, (rows, cols)), shape=(sph_vs.shape[0], 24)).todense())
  return sph_weights
